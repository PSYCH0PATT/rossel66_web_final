...
## B-10. Подвал страницы рисует не shell

`<DashboardFooter />` вызывается вручную из семи файлов
([admin-dashboard-client](../app/dashboard/admin/dashboard/admin-dashboard-client.tsx),
[admin-artists-client](../app/dashboard/admin/artists/admin-artists-client.tsx),
[artist-reports](../components/artist-reports.tsx) и др.), а не из
`DashboardShell` — то есть на части экранов его просто нет. Это остаток F-30:
C-01 обещал «футер — в общий shell», и шапку в общий компонент вынесли, а футер
так и остался на страницах.
...
**Итого: 11 однотипных расхождений.** Самый дорогой по объёму — B-02
...
это чистка мёртвых классов. B-04 и B-10 — не косметика: на этих экранах
пользователь остаётся без заголовка и навигации.
