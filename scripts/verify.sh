...
# C-01: страница не задаёт свою ширину, поля и подвал — это делает
# DashboardShell. Правило про Tailwind-классы на КОРНЕВОМ элементе страницы
# селектором ESLint не выражается (у esquery нет понятия «корень возврата
# компонента»), поэтому оно живёт отдельным скриптом.
step "Рамка страниц (C-01)"
pnpm check:page-shell || fail "страница задаёт свою ширину или поля"
