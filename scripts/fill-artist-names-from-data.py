#!/usr/bin/env python3
"""
Заполнение artistName из zvonko_data и koala_data для всех релизов
...
def main():
    print('🔧 Заполнение artistName из парсеров для всех релизов...\n')
    ...
    for release in releases:
        ...
        # 2. Если нет zvonko_data, проверяем другие источники (koalaId и т.д.)
        if not artist_name:
            # Можно проверить другие поля, но пока фокус на zvonko_data
            pass
        
        current_artist_name = release.get('artistName', '').strip()
        
        # Если нашли имя артиста из парсера и оно отличается от текущего (или пустое)
        if artist_name:
