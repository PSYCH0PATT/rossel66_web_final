...
## Деплой на сервер (Timeweb и др.)

**Важно:** на сервере обязательно задайте переменную окружения **`DATABASE_URL`** (та же строка подключения, что и в `.env.local`). Без неё приложение не подключится к Supabase: логин не будет работать, данные не загрузятся.

1. В панели хостинга (Timeweb) откройте настройки проекта/приложения.
2. Найдите раздел **Переменные окружения** (Environment variables).
3. Добавьте переменную:
   - Имя: `DATABASE_URL`
   - Значение: ваша строка подключения к Supabase (как в `.env.local`), например:
     `postgresql://postgres.xxxx:ПАРОЛЬ@aws-0-eu-central-1.pooler.supabase.com:5432/postgres`
4. Сохраните и перезапустите/пересоберите приложение.

Если логин работает локально, но на сервере нет — в 99% случаев на сервере не задан или не подхвачен `DATABASE_URL`.
