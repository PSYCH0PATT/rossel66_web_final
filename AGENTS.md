...
гейт. У линтера остаются 29 старых предупреждений (в основном `<img>` вместо `next/image` и
зависимости хуков); `no-restricted-syntax` из UI-enforcement не срабатывает больше нигде —
волна 4.4 закрыла последний файл из списка. Ошибок нет.
...
- **Enforcement — ESLint.** `.eslintrc.json` держит `no-restricted-syntax` для
  `app/dashboard/**/*.tsx`: raw-элементы и arbitrary-цвета/inline-color — `error`, без
  исключений. Поимённый список старых нарушителей (второй `overrides`-блок на `warn`) исчерпан
  волнами этапа 4 и удалён; если понадобится завести временное послабление, помните про
  glob-экранирование скобок в путях с `[id]`/`[username]` (`\\[id\\]`) — иначе override молча
  не матчится и файл остаётся на `error`.
