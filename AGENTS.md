...
- **Рамку и заголовок задаёт shell, а не страница.** Ширина, поля и подвал —
  ...
  соседних экранах брались именно оттуда. Возврат на уровень выше —
  ...
  а не `cn()`. tailwind-merge держит `text-balance` и `text-4xl` в одной группе
  `text-*` и молча выкидывает первый; пока размер приходил вторым аргументом
  `cn()`, `text-balance` вообще не доезжал до разметки, и фикс F-83 (перенос
  «РЕДАКТИРОВАНИ / Е» на 390) в бою не работал.
  Стерегут двое: `pnpm check:page-shell` (входит в `pnpm verify`) роняет сборку
