...
`pnpm lint` **не работает** — конфига ESLint в репозитории нет, `next lint` уходит в
интерактивный мастер настройки. Не пытайтесь чинить это мимоходом.

`tsc --noEmit` даёт **две известные ошибки** — `lib/prisma.ts:10` и
...
**Два планировщика включены одновременно.** `.env.example:70` ставит
`ENABLE_IN_PROCESS_SCHEDULER=true`, а `entrypoint.sh` запускает `crond` безусловно. Парсеры
запускаются дважды. Прежде чем добавлять задачу, посмотрите, нет ли её уже в `crontab`.
...
`middleware.ts`, seed для Prisma, конфиг ESLint. Тесты покрывают только формы, Buildin и
форматирование — дашборд, релизы, отчёты, плейлисты и парсеры не покрыты, зелёный билд там
означает лишь «типы сошлись».
