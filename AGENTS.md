...
гейт. У линтера остаются 190 предупреждений (30 старых — в основном `<img>` вместо `next/image`
и зависимости хуков — плюс 161 `no-restricted-syntax` из UI-enforcement, см. ниже); ошибок нет.
Правило `react/no-unescaped-entities` выключено намеренно: интерфейс русскоязычный, и
экранирование кавычек в тексте сделало бы исходники нечитаемыми.
...
Кабинеты (`app/dashboard/**`) идут по плейбуку `docs/ui-audit.md`: 2.1 токены → 2.2 компоненты →
3 ESLint-enforcement → 4 миграция волнами → 5 перестановки по вердиктам lk-cro. Перед любой
правкой в `app/dashboard/**` открыть три документа: [/dev/ui](app/dev/ui) (живая витрина
компонентов кита C-01…C-17), `docs/ui-audit.md` (системные причины и волны) и
`docs/ia-decisions.md` (вердикты по иерархии/размещению).

Правила:
- **Только `components/ui`.** Никаких raw `<button>/<input>/<select>/<table>` в `app/dashboard/**`
  — берите канонические компоненты кита. Новые страницы начинаются с **PageHeader**
  (+ **Toolbar**, если у экрана есть действия) — не пишите h1/breadcrumb/actions руками.
  Компоненты добавляются/удаляются только через `npx shadcn add <name>` (см. «Ловушки» ниже) —
  сам кит руками не форкать.
- **Цвета и отступы — только токенами.** Никаких `bg-[#…]/text-[#…]/border-[#…]` и inline
  `style={{ color/background/... }}` — используйте цветовые токены из `tailwind.config`.
  Ни строчки не добавлять в `globals.css` — новые стили живут в токенах/компонентах.
- **Enforcement — ESLint.** `.eslintrc.json` держит `no-restricted-syntax` для
  `app/dashboard/**/*.tsx`: raw-элементы и arbitrary-цвета/inline-color — `error` для новых
  файлов. Уже существующие нарушители (собраны по `docs/ui-audit.md`, раздел «3 Enforcement»)
  перечислены поимённо во втором `overrides`-блоке на `warn` — их закрывают волны этапа 4, из
  списка не убирать, пока экран не смигрировал. Список путей с `[id]`/`[username]` требует
  glob-экранирования скобок (`\\[id\\]`) — иначе override молча не матчится и файл остаётся на
  `error`.
...
**У группы связанных профилей один кабинет.** Страницы привязанного редиректят на главного,
логин привязанного выдаёт сессию главного, админский список привязанных не показывает. Если
добавляете вкладку кабинета — читайте по `getArtistGroupIds(session.id)`, а не по одному id:
именно на этом обжёгся `/api/releases`, где стояло `artistId === session.id` и кабинет группы
получал 403 на релизы привязанного профиля.
...
**`git push origin staging` толкает ветку `staging`, а не текущую.** Если коммит сделан на `dev`,
команда вернёт ноль и напишет «Everything up-to-date» — толкать нечего, ошибки нет, а коммит
никуда не уехал. Так уже было. Проверяйте `git branch --show-current` перед пушем или пушьте
через `git push origin HEAD:staging`.

**Лишний ключ в `vercel.json` роняет деплой, а не сборку.** Коммит уезжает, CI зелёный, а на
стейдж ничего не попадает — молча. Так уже было с комментарием `"//"` в конфиге: два деплоя
подряд не состоялись, и заметили это только по неработающей странице. После правки `vercel.json`
проверяйте, что деплой действительно прошёл (`gh api repos/.../commits/<sha>/status`).
Белый список ключей стережёт `lib/vercel-config.test.ts`.

**Стейджинг больше НЕ смотрит в боевую базу.** У него свой проект Supabase
(`ipwmpkaxcftvnovcbmim`) со снимком прода. Снимок одноразовый — данные со временем расходятся;
освежить: `pnpm db:clone-to-staging` и `pnpm storage:clone-to-staging`. Не чините расхождение
руками в боевой базе.
...
**Скрипт, который сам грузит `.env.e2e`, не должен импортировать `lib/prisma`.** ES-импорты
вычисляются до первой строки кода: клиент поднимается на `DATABASE_URL` из `.env.local`
(боевой Supabase) ещё до того, как `loadEnvFile` подставит адрес стенда, — и запись уходит
в прод при полностью «тестовом» `process.env`. Так уже было: 20.08.2026 пять строк
`Activity` ушли в боевую базу и были удалены вручную. Подключайтесь к стенду через `pg` с
явной строкой из `.env.e2e` (так делает `scripts/visual-baseline.ts`) или поднимайте
клиент динамическим `await import()` после загрузки окружения.
...
**Отчёты собираются TypeScript'ом, а не питоном.** `lib/report-processing/` — порт прежнего
`python-report-processor.py`, сделанный дословно: округление банковское (`pyRound`), ключ
группировки треков из четырёх полей, доля присваивается а не накапливается. Любая правка обязана
пройти `lib/report-processing/golden.test.ts` — он сверяет результат с эталоном питона ячейка в
ячейку. Эталон (`tests/fixtures/golden-report.json`) пересобирать нельзя, не убедившись, что
изменение поведения намеренное: это деньги артистов.
