...
test.describe.serial("связанные профили", () => {
  ...
  test("в подписи артиста стоит владелец релиза, а не владелец кабинета", async ({ request }) => {
    ...
    // Колонки «Артисты» в кабинете артиста больше нет (вердикт 3.3), но то же
    // поле кормит строку артистов на карте релиза — контракт API прежний.
    const response = await getAs(
      request,
      USERS.main,
      `/api/releases?artistId=${USERS.main.id}&pageSize=100`
