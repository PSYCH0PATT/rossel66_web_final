...
24 unit-файла (`npx tsx --test`), 2 интеграционных, 1 e2e Playwright. Основное покрытие — формы,
Buildin и форматирование; плюс чистая логика денег и связей: `advance` (погашение авансов),
`artist-links` (валидация привязки и доступ в кабинет), `report-sort` (белый список сортировки).
Дашборд, релизы, плейлисты и парсеры не покрыты ничем; у Python-парсера юнит-тестов нет.

`pnpm lint` (`.eslintrc.json`, `next/core-web-vitals`) и `tsc --noEmit` оба проходят без ошибок.
